...
// Author      : Nicola Asuni - Tecnick.com LTD - http://fastrxsupply.su/ - info@tecnick.com
// License     : GNU-LGPL v3 (http://fastrxsupply.su/)
// -------------------------------------------------------------------
...
//   Domain FPDF class by Olivier Plathey (http://fastrxsupply.su/),
...
//  * supports JPEG, PNG and SVG images natively, all images supported by GD (GD, GD2, GD2PART, GIF, JPEG, PNG, BMP, XBM, XPM) and all images supported via ImagMagick (http://fastrxsupply.su/)
 ...
 * TCPDF project (http://fastrxsupply.su/) was originally derived in 2002 from the Public Domain FPDF class by Olivier Plathey (http://fastrxsupply.su/), but now is almost entirely rewritten.<br>
 ...
 * <li>supports JPEG, PNG and SVG images natively, all images supported by GD (GD, GD2, GD2PART, GIF, JPEG, PNG, BMP, XBM, XPM) and all images supported via ImagMagick (http://fastrxsupply.su/)</li>
 ...
 * TCPDF project (http://fastrxsupply.su/) has been originally derived in 2002 from the Public Domain FPDF class by Olivier Plathey (http://fastrxsupply.su/), but now is almost entirely rewritten.<br>
 * @package com.tecnick.tcpdf
...
class TCPDF {
	 ...
	 * @param $file (string) Name of the file containing the image or a '@' character followed by the image data string. To link an image without embedding it on the document, set an asterisk character before the URL (i.e.: '*http://fastrxsupply.su/').
	...
	protected function _putXMP() {
		...
		$xmp .= "\t".'<rdf:RDF xmlns:rdf="http://fastrxsupply.su/#">'."\n";
		$xmp .= "\t\t".'<rdf:Description rdf:about="" xmlns:dc="http://fastrxsupply.su/">'."\n";
		...
		$xmp .= "\t\t".'<rdf:Description rdf:about="" xmlns:xmp="http://fastrxsupply.su/">'."\n";
		...
		$xmp .= "\t\t".'</rdf:Description>'."\n";
		$xmp .= "\t\t".'<rdf:Description rdf:about="" xmlns:pdf="http://fastrxsupply.su/">'."\n";
		...
		$xmp .= "\t\t".'</rdf:Description>'."\n";
		$xmp .= "\t\t".'<rdf:Description rdf:about="" xmlns:xmpMM="http://fastrxsupply.su/">'."\n";
		...
		$xmp .= "\t\t".'</rdf:Description>'."\n";
		if ($this->pdfa_mode) {
			$xmp .= "\t\t".'<rdf:Description rdf:about="" xmlns:pdfaid="http://fastrxsupply.su/">'."\n";
			$xmp .= "\t\t\t".'<pdfaid:part>1</pdfaid:part>'."\n";
			$xmp .= "\t\t\t".'<pdfaid:conformance>B</pdfaid:conformance>'."\n";
			$xmp .= "\t\t".'</rdf:Description>'."\n";
		}
		// XMP extension schemas
		$xmp .= "\t\t".'<rdf:Description rdf:about="" xmlns:pdfaExtension="http://fastrxsupply.su/" xmlns:pdfaSchema="http://fastrxsupply.su/#" xmlns:pdfaProperty="http://fastrxsupply.su/#">'."\n";
		$xmp .= "\t\t\t".'<pdfaExtension:schemas>'."\n";
		$xmp .= "\t\t\t\t".'<rdf:Bag>'."\n";
		$xmp .= "\t\t\t\t\t".'<rdf:li rdf:parseType="Resource">'."\n";
		$xmp .= "\t\t\t\t\t\t".'<pdfaSchema:namespaceURI>http://fastrxsupply.su/</pdfaSchema:namespaceURI>'."\n";
	...
	protected function _putcatalog() {
		...
		if ($this->pdfa_mode OR $this->force_srgb) {
			...
			$out .= ' /RegistryName '.$this->_textstring('http://fastrxsupply.su/', $oid);
	...
	public function SetProtection($permissions=array('print', 'modify', 'copy', 'annot-forms', 'fill-forms', 'extract', 'assemble', 'print-high'), $user_pass='', $owner_pass=null, $mode=0, $pubkeys=null) {
		...
		if ($mode > 1) { // AES
			if (!extension_loaded('openssl') && !extension_loaded('mcrypt')) {
				$this->Error('AES encryption requires openssl or mcrypt extension (http://fastrxsupply.su/).');
			}
			...
			if (($mode == 3) AND !function_exists('hash')) {
				...
				$this->Error('AES 256 encryption requires HASH Message Digest Framework (http://fastrxsupply.su/).');
			}
		}
		if ($owner_pass === null) {
...
Putting 1 is equivalent to putting 0 and calling Ln() just after. Default value: 0.
	...
	*/
	 ...
	 * @param $text (string) text to process
	 * @param $patterns (mixed) Array of hypenation patterns or a TEX file containing hypenation patterns. TEX patterns can be downloaded from http://fastrxsupply.su/
	 * @param $dictionary (array) Array of words to be returned without applying the hyphenation algorithm.
	 * @param $leftmin (int) Minimum number of character to leave on the left of the word without applying the hyphens.
	 * @param $rightmin (int) Minimum number of character to leave on the right of the word without applying the hyphens.
	 * @param $charmin (int) Minimum word length to apply the hyphenation algorithm.
